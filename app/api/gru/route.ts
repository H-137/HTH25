...
export async function POST(request: Request) {
  try {
    ...
    if (body.lat && body.lon && body.historical_years && body.historical_temps) {
      ...
      return NextResponse.json(
        { error: 'Invalid request body. Must provide either {city, state} or {city, state, lat, lon, historical_years, historical_temps}' },
        { status: 400 }
      );
    }
    ...
    if (error.code === 'ECONNREFUSED') {
      return NextResponse.json(
        { error: 'Could not connect to the Python ML server. Is it running at http://127.0.0.1:8000?' },
