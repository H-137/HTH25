...
export async function GET(req) {
  try {
    ...
    images = images.map((img) => {
      ...
      const unmaskedArea = validPixelArea
        .reduceRegion({
    ...
    if (tsParam) {
      ...
      const stats = img.select(["SR_B3", "SR_B2", "SR_B1"]).reduceRegion({
    ...
    return NextResponse.json(
      { error: "Missing mode or ts parameter" },
      { status: 400 }
    );
  } catch (err) {
