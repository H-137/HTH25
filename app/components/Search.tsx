...
export default function Search({ setLocation }: { setLocation: (location: { lat: number; long: number }) => void }) {
    ...
    const [coords, setCoords] = useState<{ lat: number; long: number } | null>(null) // coords from API
    ...
    try {
        ...
        const apiCity = String(data.name || "").trim().toLowerCase()
        const abbr = stateInit.trim().toUpperCase()
        const full = abbr ? US_STATE_BY_ABBR[abbr] || "" : ""
        const apiAdmin = String(data.admin1 || "").trim().toLowerCase()
