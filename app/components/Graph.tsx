...
export default function Graph({ title, average, coords }: { title: string, average: boolean, coords?: Coords }) {
  ...
  useEffect(() => {
    ...
    if (Papa) {
      Papa.parse('/static/temps.csv', {
        ...
        header: true,
        dynamicTyping: true,
        complete: (results: any) => { // Use 'any' type as Papa is not imported
      ...
      console.error("PapaParse (Papa) library not found on window object. CSV will not load.");
    ...
    ;(async () => {
      try {
        ...
        if (!res.ok || data?.error) {
          ...
          const errorMsg = (data?.error && typeof data.error === 'string') ? data.error : JSON.stringify(data?.error) || "failed to fetch historical series";
  ...
  if (isCityMode) {
    ...
    const paddedMainSeries = [...historicalData, ...Array(futureLabels.length).fill(null)];
    ...
    if (average) {
      ...
      const paddedAvgSeries = [...historicalAvgData, ...Array(futureLabels.length).fill(null)];
      chartDatasets.push({
    ...
    if (prediction?.predictions) {
      ...
      const predictionSeries = [...Array(historicalLabels.length).fill(null), ...predictionData];
      chartDatasets.push({
    ...
    const csvMainSeries = csvData.map(row => row["Annual Average Temperature (F)"]);
    ...
    <div>
      ...
      {isCityMode ? (
        ...
        <>
          ...
          {!apiLoading && apiError && <p style={{ color: 'red' }}>Error loading historical data: {apiError}</p>}
          {!apiLoading && !apiError && yearly.length > 0 && (
            <>
              ...
              <div style={{ marginTop: '20px', padding: '10px', border: '1px solid #ccc', borderRadius: '8px' }}>
                ...
                {predictionError && <p style={{ color: 'red' }}>Prediction Error: {predictionError}</p>}
                {prediction && (
                  <div>
                    <div style={{ marginBottom: '0' }}>
                      <strong>Predicted Avg Temp (2026-{prediction.predictions.years.slice(-1)[0] || '2045'}):</strong>
                      {/* MODIFIED: Use the specific type */}
                      {prediction.predictions && prediction.predictions.avg_temp ?
